...
| 8 | Refund naplate pretplate (i proracione) | Dashboard → refund `ch_` | `charge.refunded` → `invoicePayments` nađe `in_` → `apply_refund` −(suma `delta` redova pod `in_`): **jedan** red `povracaj`, ref `povracaj:<ch_…>`, bez `admin_audit`. Pro 450 + upgrade na Advanced (+750) pa refund → −750, balans 450. Resend istog događaja → `duplikat`; bez markera → `already_applied`, nula novih redova. Balans može u minus do −1000 |
