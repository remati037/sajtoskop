...
3. **Krediti se menjaju samo kroz `spend_credit_and_unlock`, `spend_credit_and_scan` ili `grant_credits`.** Nikad direktan `UPDATE profiles.credits_balance`. (`grant_monthly_credits` i `refund_scan(job, pages)` su izuzeci objašnjeni u migracijama 0004, 0009 i 0025 — `refund_scan` od S25 vraća i **razliku** kad Google da manje stranica nego što je plaćeno.) Od 0034 `refund_scan` piše razlog **`scan_refund`** (ref `scan_refund:<job_id>`, `details` nosi posao i kombinaciju), briše `search_access` za taj posao kod punog povraćaja i upisuje red u `admin_audit`; stari oblik (`refund` + `scan:<job_id>`) i dalje važi kao dokaz da je posao refundiran. Skeniranje koje ne da listu obara posao i vraća kredit u JEDNOJ transakciji, kroz `fail_scan_and_refund` — korisnik ne prijavljuje ništa i ponavljanje pretrage je dozvoljeno. Od F11/F12 postoje još dva omotača, oba `security definer` i oba samo za `service_role`: `grant_feedback_credits` (0011) i `admin_adjust_credits` (0012 — **jedini put za negativan iznos**, jer `grant_credits` po definiciji odbija negativan). Od S25 (0025) Stripe webhook ide isključivo kroz `apply_subscription` (stanje, bez kredita), `apply_invoice_paid` (mesečna dodela, ref `in_…`), `apply_trial_start` (10 probnih, jednom po nalogu), `expire_subscription_credits` (pražnjenje na `subscription.deleted`) i `apply_credit_pack` (paket, ref `pi_…`); komp pristup kroz `admin_open_komp` ili `redeem_invite`. `grant_credits` interno validira `reason`; nov razlog znači izmenu i `check` ograničenja i tela funkcije. Od 0029 obe `spend_*` funkcije nalogu sa `profiles.role = 'admin'` ne skidaju kredite i ne pišu knjigu — jedini izuzetak od naplate, samo po ulozi iz baze (ne po `ADMIN_BOOTSTRAP_IDS`); dnevni osigurač i Places budžet za admina važe. Od 0030 `grant_monthly_credits` upisuje `credit_ledger.balance_after` (= target), a povraćaj mesečne dodele skida **ceo mesec** iz tog polja, ne deltu; `admin_adjust_credits(kind=povracaj)` odseca na pod −1000 (`odseceno` / `na_podu`), traženo i skinuto ostaju u reviziji. Od 0032 **vraćen novac** (`charge.refunded`, izgubljen spor) ide isključivo kroz `apply_refund` (`security definer`, samo `service_role`): jedan red `reason = 'povracaj'`, `ref_id` tačno `povracaj:<ch_…>` / `spor:<dp_…>` bez sufiksa, bez granice od 500; iznos je **suma `delta` redova knjige pod `in_…` / `pi_…` te naplate** (srazmerno delimičnom refundu, nadole) — nikad iz `plans.ts`, nikad iz `balance_after`, bez redova se ne skida ništa. Paket skida iz dopune, pretplata samo iz balansa; pod −1000 i dalje važi. Od 0033 je ključ **ID refunda** (`povracaj:<re_…>`, iz `refunds.list`, nikad iz `charge.refunds`): retry pada na indeks, a dva delimična refunda iste naplate su dva reda sa svojim iznosom (`refund.amount / charge.amount`). Preliv paketa u balans piše `admin_audit` red `refund_preliv` sa `iz_dopune` i `iz_balansa`.
