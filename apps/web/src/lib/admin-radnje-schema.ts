 ...
 * po kome `planFor()` tiho pada na `beta`, pa bi konzola tvrdila jedno a
 * proizvod radio drugo.
 */
export const planBodySchema = z.strictObject({
  plan: z.enum(Object.keys(PLANS) as [string, ...string[]], {
    error: `Nepoznat plan. Dozvoljeni su: ${Object.keys(PLANS).join(", ")}.`,
  }),
});

export const PLAN_OPCIJE = Object.keys(PLANS);
